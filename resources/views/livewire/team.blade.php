        ...
        <section class="mx-auto max-w-7xl">
                ...
                <div class="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
                    @forelse ($miembros as $m)
                        <article data-team-member class="group rounded-md border border-cyan-400/10 bg-[#07101f] p-8 text-center transition hover:-translate-y-1 hover:border-cyan-300/40">
                            <div class="relative mx-auto mb-5 h-28 w-28">
                                <img
                                    src="{{ $m->foto && \Illuminate\Support\Facades\Storage::disk('public')->exists($m->foto) ? asset('storage/'.$m->foto) : 'https://ui-avatars.com/api/?name='.urlencode($m->nombre).'&background=06b6d4&color=030712' }}"
                                    alt="{{ $m->nombre }}"
                            ...
                            <div class="mt-2 flex justify-center gap-4">
                                @if ($m->linkedin)
                                    <a href="{{ $m->linkedin }}" target="_blank" rel="noopener noreferrer" class="text-slate-500 transition hover:text-cyan-300">
                                ...
                                @endif
                                @if ($m->github)
                                    <a href="{{ $m->github }}" target="_blank" rel="noopener noreferrer" class="text-slate-500 transition hover:text-cyan-300">
