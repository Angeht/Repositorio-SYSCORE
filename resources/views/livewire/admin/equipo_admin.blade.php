                ...
                <div class="mb-8 rounded-md border border-cyan-400/10 bg-black/20 p-5">
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="file" wire:model="foto" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" class="w-full text-slate-400">
                            ...
                            @elseif ($currentFoto && \Illuminate\Support\Facades\Storage::disk('public')->exists($currentFoto))
                                <img src="{{ asset('storage/'.$currentFoto) }}" class="mt-3 h-16 w-16 rounded-full object-cover ring-2 ring-cyan-400/30">
                            @endif
                        </div>
                    </div>
                ...
                @forelse ($miembros as $m)
                    <div class="flex items-start justify-between rounded-md border border-cyan-400/10 bg-black/20 p-5 transition hover:border-cyan-400/30">
                        <div class="flex items-start gap-4">
                            @if ($m->foto && \Illuminate\Support\Facades\Storage::disk('public')->exists($m->foto))
