<div class="min-h-screen bg-[#030712] text-slate-100">
    ...
    <main class="mx-auto grid max-w-7xl gap-6 px-5 py-8 lg:grid-cols-[340px_1fr] lg:px-10">
        ...
        <aside class="rounded-md border border-cyan-400/10 bg-[#07101f] p-4">
            ...
            <div class="space-y-2">
                ...
                <a href="{{ route('admin.librerias') }}"
                    class="flex w-full items-center rounded-md bg-white/[0.03] px-4 py-3 text-slate-300 transition hover:bg-cyan-400/10 hover:text-cyan-200">
                    <div>
                        <span class="block text-xs font-black uppercase tracking-[0.2em]">librerias</span>
                        <span class="mt-1 block text-sm font-semibold">Gestión</span>
                    </div>
                </a>
                <a href="{{ route('admin.libreriacss') }}"
                    class="flex w-full items-center rounded-md bg-cyan-400 px-4 py-3 text-[#05111f]">
                    <div>
                        <span class="block text-xs font-black uppercase tracking-[0.2em]">librerias css</span>
                        <span class="mt-1 block text-sm font-semibold">Gestión</span>
                    </div>
                </a>
            </div>
        ...
        <section class="rounded-md border border-cyan-400/10 bg-[#07101f] p-6 md:p-8">
            ...
            <div class="mb-8 rounded-md border border-cyan-400/10 bg-black/20 p-5">
                <p class="mb-4 text-xs font-bold uppercase tracking-widest text-cyan-300">
                    {{ $Id ? 'Editar libreria css' : 'Nueva libreria css' }}
                </p>

                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">Libreria Css</label>
                        <input type="text" wire:model="descripcion_libreriacss"
                        ...
                        @error('descripcion_libreriacss') <span class="mt-1 text-xs text-red-400">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">Imagen</label>
                        <input type="file" wire:model="ruta_libreriacss" class="w-full text-slate-400">
                        @error('ruta_libreriacss') <span class="mt-1 text-xs text-red-400">{{ $message }}</span> @enderror
                        @if ($ruta_libreriacss)
                        <img src="{{ $ruta_libreriacss->temporaryUrl() }}" class="mt-3 h-16 w-16 rounded-full object-cover ring-2 ring-cyan-400/30">
                        @elseif($currentImg)
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div class="flex items-start justify-between rounded-md border border-cyan-400/10 bg-black/20 p-5 transition hover:border-cyan-400/30">
                    <div class="flex items-start gap-4">
                        @if ($l->ruta_libreriacss)
                        <img src="{{ asset('storage/'.$l->ruta_libreriacss) }}" class="h-12 w-12 rounded-md object-cover ring-2 ring-cyan-400/20">
                        @else
                        <div class="flex h-12 w-12 items-center justify-center rounded-md bg-cyan-400/10 text-sm font-black text-cyan-300 ring-2 ring-cyan-400/20">
                            {{ collect(explode(' ', $l->descripcion_libreriacss))->map(fn($l) => mb_substr($l, 0, 1))->take(2)->implode('') }}
                        </div>
                        @endif
                        <div>
                            <p class="text-sm text-slate-400">{{ $l->descripcion_libreriacss }}</p>
                        </div>
                    </div>
                    <div class="flex flex-col items-end gap-2 text-xs">
                        <button wire:click="actualizar({{ $l->idlibreriacss }})" class="font-semibold text-cyan-400 transition hover:text-cyan-300">Editar</button>
                        <button wire:click="confirmarEliminar({{ $l->idlibreriacss }})"
                            class="font-semibold text-red-400 transition hover:text-red-300">Eliminar</button>
                    </div>
                </div>
                @empty
                <div class="col-span-full py-12 text-center">
                    <p class="text-slate-500">Aún no hay librerias css registradas.</p>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
        <div class="w-full max-w-xs rounded-md border border-red-400/20 bg-[#07101f] p-5 text-center shadow-[0_0_30px_rgba(248,113,113,.1)]">
            ...
            <p class="text-sm font-bold text-white">¿Eliminar a {{ $delete }}?</p>
