        ...
        <section class="rounded-md border border-cyan-400/10 bg-[#07101f] p-6 md:p-8">
            ...
            <div class="mb-8 rounded-md border border-cyan-400/10 bg-black/20 p-5">
                <p class="mb-4 text-xs font-bold uppercase tracking-widest text-cyan-300">
                    {{ $Id ? 'Editar libreria' : 'Nueva libreria' }}
                </p>

                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">Libreria</label>
                        <input type="text" wire:model="descripcion_libreria"
                        ...
                        @error('descripcion_libreria') <span class="mt-1 text-xs text-red-400">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase tracking-wider text-slate-500">Imagen</label>
                        <input type="file" wire:model="ruta_libreria" class="w-full text-slate-400">
                        @error('ruta_libreria') <span class="mt-1 text-xs text-red-400">{{ $message }}</span> @enderror
                        @if ($ruta_libreria)
                        <img src="{{ $ruta_libreria->temporaryUrl() }}" class="mt-3 h-16 w-16 rounded-full object-cover ring-2 ring-cyan-400/30">
                        @elseif($currentImg)
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div class="flex items-start justify-between rounded-md border border-cyan-400/10 bg-black/20 p-5 transition hover:border-cyan-400/30">
                    <div class="flex items-start gap-4">
                        @if ($l->ruta_libreria)
                        <img src="{{ asset('storage/'.$l->ruta_libreria) }}" class="h-12 w-12 rounded-md object-cover ring-2 ring-cyan-400/20">
                        @else
                        <div class="flex h-12 w-12 items-center justify-center rounded-md bg-cyan-400/10 text-sm font-black text-cyan-300 ring-2 ring-cyan-400/20">
                            {{ collect(explode(' ', $l->descripcion_libreria))->map(fn($l) => mb_substr($l, 0, 1))->take(2)->implode('') }}
                        </div>
                        @endif
                        <div>
                            <p class="text-sm text-slate-400">{{ $l->descripcion_libreria }}</p>
                        </div>
                    </div>
                    <div class="flex flex-col items-end gap-2 text-xs">
                        <button wire:click="actualizar({{ $l->idlibreria }})" class="font-semibold text-cyan-400 transition hover:text-cyan-300">Editar</button>
                        <button wire:click="confirmarEliminar({{ $l->idlibreria }})"
                            class="font-semibold text-red-400 transition hover:text-red-300">Eliminar</button>
                    </div>
                </div>
                @empty
                <div class="col-span-full py-12 text-center">
                    <p class="text-slate-500">Aún no hay librerias registradas.</p>
